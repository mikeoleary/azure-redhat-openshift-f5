...
#Get the subnet CIDR blocks of ARO masters and workers
MASTER_SUBNET_IP_CIDR=$(az network vnet subnet show --ids $ARO_MASTER_SUBNET_ID | jq .addressPrefix -r)
WORKER_SUBNET_IP_CIDR=$(az network vnet subnet show --ids $ARO_WORKER_SUBNET_ID | jq .addressPrefix -r)
...
curl -s -k -X POST -H "Content-Type: application/json" -u admin:$BIGIP_MGMT_PASSWORD https://$BIGIP_MGMT_ADDRESS/mgmt/tm/net/route/ -d '{"name": "routeMasterNodes", "partition": "Common", "gw": "'"$BIGIP_INT_SELFIP"'", "network": "'"$MASTER_SUBNET_IP_CIDR"'"}'
curl -s -k -X POST -H "Content-Type: application/json" -u admin:$BIGIP_MGMT_PASSWORD https://$BIGIP_MGMT_ADDRESS/mgmt/tm/net/route/ -d '{"name": "routeWorkerNodes", "partition": "Common", "gw": "'"$BIGIP_INT_SELFIP"'", "network": "'"$WORKER_SUBNET_IP_CIDR"'"}'
