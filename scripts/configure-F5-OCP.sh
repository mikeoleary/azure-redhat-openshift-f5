...
## THIS SCRIPT REQUIRES ENVIRONMENT VARIABLES OF THE FOLLOWING:
#ARO_CLUSTER_NAME
#ARO_CLUSTER_RESOURCE_GROUP
#BIGIP_MGMT_ADDRESS0
#BIGIP_MGMT_ADDRESS1
#BIGIP_VSERVER_ADDR
#BIGIP_INT_FLOAT_IP
...
BIGIP_INT_SELFIP_WITH_MASK=$(curl -s -k -H "Content-Type: application/json" -u admin:$BIGIP_MGMT_PASSWORD https://$BIGIP_MGMT_ADDRESS0/mgmt/tm/net/self/self_3nic | jq .address -r)
...
curl -s -k -X POST -H "Content-Type: application/json" -u admin:$BIGIP_MGMT_PASSWORD https://$BIGIP_MGMT_ADDRESS0/mgmt/tm/net/self -d '{"name": "internalSelfFloat", "address": "'"$BIGIP_INT_FLOAT_IP_WITH_MASK"'", "vlan": "internal", "traffic-group":"traffic-group-1", "allowService": ["default", "tcp:4789"] }'
#Get Internal vlan gw
BIGIP_INT_GW="${BIGIP_INT_SELFIP%.*}.1"
#Get private IP for Mgmt 
BIGIP_MGMT_PRIVATE_IP_WITH_MASK=$(curl -s -k -H "Content-Type: application/json" -u admin:$BIGIP_MGMT_PASSWORD https://$BIGIP_MGMT_ADDRESS0/mgmt/tm/sys/management-ip | jq .items[0].fullPath -r)
read -a strarr <<< $(echo $BIGIP_MGMT_PRIVATE_IP_WITH_MASK | tr "/" " ")
BIGIP_MGMT_PRIVATE_IP=${strarr[0]}

##BIGIP DEVICE1
#Get private IP for Internal SelfIP
BIGIP_INT_SELFIP_WITH_MASK1=$(curl -s -k -H "Content-Type: application/json" -u admin:$BIGIP_MGMT_PASSWORD https://$BIGIP_MGMT_ADDRESS1/mgmt/tm/net/self/self_3nic | jq .address -r)
read -a strarr <<< $(echo $BIGIP_INT_SELFIP_WITH_MASK1 | tr "/" " ")
BIGIP_INT_SELFIP1=${strarr[0]}
BIGIP_INT_SELFIP_MASK1=${strarr[1]}
#Get Internal vlan gw
BIGIP_INT_GW1="${BIGIP_INT_SELFIP1%.*}.1"
#Get private IP for Mgmt 
BIGIP_MGMT_PRIVATE_IP_WITH_MASK1=$(curl -s -k -H "Content-Type: application/json" -u admin:$BIGIP_MGMT_PASSWORD https://$BIGIP_MGMT_ADDRESS1/mgmt/tm/sys/management-ip | jq .items[0].fullPath -r)
read -a strarr <<< $(echo $BIGIP_MGMT_PRIVATE_IP_WITH_MASK1 | tr "/" " ")
BIGIP_MGMT_PRIVATE_IP1=${strarr[0]}
...
cp /tmp/f5-bigip-node.yaml /tmp/f5-bigip-node1.yaml
...
sed -i -e "s/<INTERNALSELFIP>/$BIGIP_INT_SELFIP1/" /tmp/f5-bigip-node1.yaml
sed -i -e "s/<NODENAME>/f5-bigip-node02/" /tmp/f5-bigip-node1.yaml
...
# and for the non-float addr for device0
...
# and for the non-float addr for device1
BIGIP_HOST_SUBNET_WITH_MASK1=$(oc get hostsubnet f5-bigip-node02 -o json | jq .subnet -r)
read -a strarr <<< $(echo $BIGIP_HOST_SUBNET_WITH_MASK1 | tr "/" " ")
BIGIP_HOST_SUBNET1=${strarr[0]}
BIGIP_TUNNEL_SELFIP1="${BIGIP_HOST_SUBNET1%.*}.100/$CLUSTERNETWORK_MASK"
...
curl -s -k -X POST -H "Content-Type: application/json" -u admin:$BIGIP_MGMT_PASSWORD https://$BIGIP_MGMT_ADDRESS0/mgmt/tm/net/self -d '{"name": "tunnelSelfIP_float", "address": "'"$BIGIP_TUNNEL_SELFIP_FLOAT"'", "vlan": "openshift_vxlan", "traffic-group": "traffic-group-1", "allowService": "all" }'
#set allowed services on selfIP of Internal VLAN to be default + tcp:4789
curl -s -k -X PATCH -H "Content-Type: application/json" -u admin:$BIGIP_MGMT_PASSWORD https://$BIGIP_MGMT_ADDRESS0/mgmt/tm/net/self/self_3nic -d '{"allowService": ["default","tcp:4789"]}'
curl -s -k -X PATCH -H "Content-Type: application/json" -u admin:$BIGIP_MGMT_PASSWORD https://$BIGIP_MGMT_ADDRESS1/mgmt/tm/net/self/self_3nic -d '{"allowService": ["default","tcp:4789"]}'

#Create routes on BIG-IP so that traffic to the Master and Worker nodes traverses the internal NIC. This only needs to be done on Device0
sleep 5
curl -s -k -X POST -H "Content-Type: application/json" -u admin:$BIGIP_MGMT_PASSWORD https://$BIGIP_MGMT_ADDRESS0/mgmt/tm/net/route -d '{"name": "routeMasterNodes", "partition": "Common", "gw": "'"$BIGIP_INT_GW"'", "network": "'"$MASTER_SUBNET_IP_CIDR"'"}'
curl -s -k -X POST -H "Content-Type: application/json" -u admin:$BIGIP_MGMT_PASSWORD https://$BIGIP_MGMT_ADDRESS0/mgmt/tm/net/route -d '{"name": "routeWorkerNodes", "partition": "Common", "gw": "'"$BIGIP_INT_GW"'", "network": "'"$WORKER_SUBNET_IP_CIDR"'"}'
...
sed -i -e "s/<bigipUrl2>/$BIGIP_MGMT_PRIVATE_IP1/" /tmp/cis.yaml
